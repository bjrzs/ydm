  ...
  <div class="pref-keybindings">
    ...
    <section class="keybindings">
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column prop="description" :label="$t('keybindings.table.description')">
        </el-table-column>
        <el-table-column prop="accelerator" :label="$t('keybindings.table.keyCombination')" width="220">
        </el-table-column>
        <el-table-column fixed="right" :label="$t('keybindings.table.options')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleEditClick(scope.$index, scope.row)" type="text" size="small" :title="$t('keybindings.actions.edit')">
              <i class="el-icon-edit"></i>
            </el-button>
            <el-button @click="handleResetClick(scope.$index, scope.row)" type="text" size="small" :title="$t('keybindings.actions.reset')">
              <i class="el-icon-refresh-right"></i>
            </el-button>
            <el-button @click="handleUnbindClick(scope.$index, scope.row)" type="text" size="small" :title="$t('keybindings.actions.unbind')">
    ...
    <section v-if="showDebugTools" class="keyboard-debug">
      <separator></separator>
      <div><strong>{{ $t('keybindings.debug.title') }}</strong></div>
      <el-button size="medium" @click="dumpKeyboardInformation">{{ $t('keybindings.debug.dumpInfo') }}</el-button>
    </section>
    <key-input-dialog
...
export default {
  ...
  methods: {
      ...
      if (this.keybindingConfigurator && this.keybindingList.length > 0) {
        this.keybindingConfigurator.save()
          .then(success => {
            if (!success) {
              notice.notify({
                title: this.$t('keybindings.messages.saveError.title'),
                type: 'error',
                message: this.$t('keybindings.messages.saveError.message')
              })
            }
          })
    ...
    handleDuplicateShortcut (id, accelerator) {
      notice.notify({
        title: this.$t('keybindings.messages.duplicateShortcut.title'),
        type: 'warning',
        message: this.$t('keybindings.messages.duplicateShortcut.message', { accelerator })
      })
    },
